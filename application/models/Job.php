...
class Job extends CI_Model
{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'desk_profesi.id', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(desk_profesi.id) as count');
		}
		else
		{
		
			$this->db->select('desk_profesi.*');
		...
		{
			$this->db->group_start();
				$this->db->like('desk_profesi.name', $search);
	...
	*/
	public function get_info($item_id)
	{
		...
		$this->db->where('desk_profesi.id', $item_id);
		...
		else
		{
			...
			foreach($this->db->list_fields('desk_profesi') as $field)
			{
				$item_obj->$field = '';
			}
	...
	*/
	public function save(&$item_data, $item_id = FALSE)
	{
		...
		{
			if($this->db->insert('desk_profesi', $item_data))
			{
		...
		return $this->db->update('desk_profesi', $item_data);
	}
	...
	*/
	public function delete($item_id)
	{
		...
		$this->db->where('id', $item_id);
		$success = $this->db->update('desk_profesi', array('deleted' => 1));
	...
	*/
	public function undelete($item_id)
	{
		...
		return $this->db->update('desk_profesi', array('deleted' => 0));
	}
	...
	*/
	...
	/*
	Checks if profesi name exists
	*/
	public function check_job_exists($profesi, $id = '')
	{
		...
		$this->db->from('desk_profesi');
		$this->db->where('desk_profesi.name', $profesi);
		//$this->db->where('desk_kota.deleted', 0);

		if(!empty($id))
		{
			$this->db->where('desk_profesi.id !=', $id);
		}
	...
	{
		$this->db->select('name as profesi');
		$this->db->from('desk_profesi');
		$this->db->where('deleted', 0);
		//$this->db->distinct();
