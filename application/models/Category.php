...
class Category extends CI_Model
{
	...
	*/
	...
	{
		return $this->search($search, $filters, 0, 0, 'desk_categories.id', 'asc', TRUE);
	}

	/*
	Perform a search on items
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'desk_categories.id', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(desk_categories.id) as count');
		}
		else
		{
		
			$this->db->select('desk_categories.id, desk_categories.name, desk_categories.desc, desk_categories.deleted');
		...
		{
			$this->db->group_start();
				$this->db->like('desk_categories.desc', $search);
			$this->db->group_end();
		}
	...
	*/
	...
	{
		$this->db->select('desk_categories.*');
		
		$this->db->from('desk_categories');
		
		$this->db->where('desk_categories.id', $item_id);
		...
		else
		{
			...
			foreach($this->db->list_fields('desk_categories') as $field)
			{
				$item_obj->$field = '';
			}
	...
	*/
	public function save(&$item_data, $item_id = FALSE)
	{
		...
		{
			if($this->db->insert('desk_categories', $item_data))
			{
		...
		return $this->db->update('desk_categories', $item_data);
	}
	...
	*/
	public function delete($item_id)
	{
		...
		$this->db->where('id', $item_id);
		$success = $this->db->update('desk_categories', array('deleted' => 1));
	...
	*/
	public function undelete($item_id)
	{
		...
		return $this->db->update('desk_categories', array('deleted' => 0));
	}
	...
	*/
	public function check_name_exists($name)
	{
		...
		$this->db->from('desk_categories');
		$this->db->where('desk_categories.name', $name);
		//$this->db->where('desk_kota.deleted', 0);

		if(!empty($id))
		{
			$this->db->where('desk_categories.id !=', $id);
		}
