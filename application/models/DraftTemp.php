...
class DraftTemp extends CI_Model
{
	/**
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'desk_drafts_temp.id', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(desk_drafts_temp.id) as count');
		}
		else
		{
			
			$this->db->select('desk_drafts_temp.id, trackid, lastchange, iden_nama, isu_topik, category');
			//$this->db->select('desk_categories.desc as jenis_produk');
			$this->db->select("date_format(tglpengaduan,'%d/%m/%Y') as tglpengaduan");
			
		}
		
		$this->db->from('desk_drafts_temp');
		//$this->db->join('desk_categories','desk_categories.id = desk_drafts_temp.kategori','left');
		...
		*/
		...
		{
			//$this->db->group_start();
				//$this->db->like('desk_drafts_temp.trackid', $search);
	...
	*/
	...
	{
		return $this->search_rujukan($search, $filters, 0, 0, 'desk_rujuk_drafts_temp.sid', 'asc', TRUE);
	}
	
	/*
	Perform a search on items
	*/
	public function search_rujukan($search, $filters, $rows = 0, $limit_from = 0, $sort = 'desk_rujuk_drafts_temp.sid', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(desk_rujuk_drafts_temp.sid) as count');
		}
		else
		{
			
			$this->db->select('desk_rujuk_drafts_temp.*');
			$this->db->select('desk_direktorat.name as unit_name, kota');
			//$this->db->select("date_format(tglpengaduan,'%d/%m/%Y') as tglpengaduan");
			
		}
		
		$this->db->from('desk_rujuk_drafts_temp');
		$this->db->join('desk_direktorat','desk_direktorat.id = desk_rujuk_drafts_temp.dir_id');
		//$this->db->where('owner', $this->session->id);
		//$this->db->where('is_sent', '0');
		//$this->db->where_in('info', array('P','I'));
	...
	*/
	public function get_info($item_id)
	{
		...
		$sql .= " FROM (SELECT * FROM desk_drafts_temp WHERE id=$item_id) a";
		...
		else
		{
			...
			foreach($this->db->list_fields('desk_drafts_temp') as $field)
			{
				$item_obj->$field = '';
			}
	...
	{
		$this->db->select('desk_ppid_drafts_temp.*,');
		...
		$this->db->from('desk_ppid_drafts_temp');
		$this->db->where('desk_ppid_drafts_temp.id', $item_id);
		...
		else
		{
			...
			//Get all the fields from items table
			foreach($this->db->list_fields('desk_ppid_drafts_temp') as $field)
			{
				$item_obj->$field = '';
			}
	...
	public function get_rujukans($item_id)
	{
		...
		$this->db->from('desk_rujuk_drafts_temp a');
	...
	{
		$this->db->where('sid', $item_id);
		$success = $this->db->delete('desk_rujuk_drafts_temp');
		...
		$this->db->update('desk_drafts_temp', $item_data);
	}
	...
	public function remove_rujukan($item_id, $dir_id)
	{
		...
		if($query->num_rows() > 0)
		{
			...
			$this->db->where('id', $item_id);		
			$this->db->update('desk_drafts_temp', $array_dir);
			...
			for($i = 0; $i<count($array); $i++)
			{
				$arr = $array[$i];
				$arr['no_urut'] = $i+1;
				$this->db->insert('desk_rujuk_drafts_temp', $arr);
				
				
				if($arr['no_urut'] == 1)
				{
					$this->db->where('id', $item_id);		
					$this->db->update('desk_drafts_temp', array('direktorat' => $arr['dir_id']));
				}
				elseif($arr['no_urut'] == 2)
				{
					$this->db->where('id', $item_id);		
					$this->db->update('desk_drafts_temp', array('direktorat2' => $arr['dir_id']));
				}
				elseif($arr['no_urut'] == 3)
				{
					$this->db->where('id', $item_id);		
					$this->db->update('desk_drafts_temp', array('direktorat3' => $arr['dir_id']));
				}
				elseif($arr['no_urut'] == 4)
				{
					$this->db->where('id', $item_id);		
					$this->db->update('desk_drafts_temp', array('direktorat4' => $arr['dir_id']));
				}
				elseif($arr['no_urut'] == 5)
				{
					$this->db->where('id', $item_id);		
					$this->db->update('desk_drafts_temp', array('direktorat5' => $arr['dir_id']));
				}
	...
	public function create_draft(&$item_data)
	{
		...
		if($this->db->insert('desk_drafts_temp', $item_data))
		{
	...
	*/
	public function save(&$item_data, $item_id = FALSE)
	{
		...
		if($item_id == -1)
		{
			...
			if($this->db->insert('desk_drafts_temp'))
			{
				$item_data['id'] = $this->db->insert_id();
		...
		$this->db->where('id', $item_id);		
		return $this->db->update('desk_drafts_temp');
	}
	...
	public function save_ppid(&$item_data, $item_id)
	{
		...
		{
			$this->db->where('id', $item_id);		
			return $this->db->update('desk_ppid_drafts_temp', $item_data);
		}
		
		return $this->db->insert('desk_ppid_drafts_temp', $item_data);
	}
	...
	public function save_to_ticket(&$item_data, $item_id = FALSE)
	{
		...
		if($query->num_rows() == 1)
		{
			...
			if($this->db->insert('desk_tickets'))
			{
				...
				$this->db->update('desk_drafts_temp');
				...
				//insert rujukan
				if($item_data['direktorat'])
				{
					$this->db->insert('desk_rujuk', array(
						'sid' => $item_data['id'],
						'dir_id' => $item_data['direktorat'],
						'no_urut' => 1,
						'sla' => $item_data['d1_prioritas'],
						'ticket_date' => $item_data['tglpengaduan']
					));
				}
				
				if($item_data['direktorat2'])
				{
					$this->db->insert('desk_rujuk', array(
						'sid' => $item_data['id'],
						'dir_id' => $item_data['direktorat2'],
						'no_urut' => 2,
						'sla' => $item_data['d2_prioritas'],
						'ticket_date' => $item_data['tglpengaduan']
					));
				}
				
				if($item_data['direktorat3'])
				{
					$this->db->insert('desk_rujuk', array(
						'sid' => $item_data['id'],
						'dir_id' => $item_data['direktorat3'],
						'no_urut' => 3,
						'sla' => $item_data['d3_prioritas'],
						'ticket_date' => $item_data['tglpengaduan']
					));
				}
				
				if($item_data['direktorat4'])
				{
					$this->db->insert('desk_rujuk', array(
						'sid' => $item_data['id'],
						'dir_id' => $item_data['direktorat4'],
						'no_urut' => 4,
						'sla' => $item_data['d4_prioritas'],
						'ticket_date' => $item_data['tglpengaduan']
					));
				}
				
				if($item_data['direktorat5'])
				{
					$this->db->insert('desk_rujuk', array(
						'sid' => $item_data['id'],
						'dir_id' => $item_data['direktorat5'],
						'no_urut' => 5,
						'sla' => $item_data['d5_prioritas'],
						'ticket_date' => $item_data['tglpengaduan']
					));
				}
	...
	*/
	public function update_multiple($item_data, $item_ids)
	{
		...
		return $this->db->update('desk_drafts_temp', $item_data);
	}
	...
	*/
	public function delete($item_id)
	{
		...
		$this->db->where('id', $item_id);
		$success = $this->db->delete('desk_drafts_temp');
		
		$this->db->where('id', $item_id);
		$success = $this->db->delete('desk_ppid_drafts_temp');
	...
	*/
	...
	public function get_unit_teknis_suggestions($search, $item_id = -1)
	{
		...
		$this->db->where("id NOT IN (SELECT dir_id FROM desk_rujuk_drafts_temp WHERE sid = $item_id)", NULL, FALSE);
