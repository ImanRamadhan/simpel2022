...
class Person extends CI_Model
{
	...
	{
		$this->db->from('m_user');
		$this->db->where('m_user.user_id', $person_id);
	...
	{
		$query = $this->db->get_where('m_user', array('user_id' => $person_id), 1);
		...
		else
		{
			...
			foreach($this->db->list_fields('m_user') as $field)
			{
				$person_obj->$field = '';
			}
	...
	{
		$this->db->from('m_user');
		$this->db->where_in('user_id', $person_ids);
		$this->db->order_by('user_id', 'asc');
	...
	public function save(&$person_data, $person_id = FALSE)
	{
		...
		{
			if($this->db->insert('m_user', $person_data))
			{
				$person_data['user_id'] = $this->db->insert_id();
		...
		return $this->db->update('m_user', $person_data);
	}
