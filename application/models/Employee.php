...
class Employee extends Person
{
	...
	/*
	Determines if a given id is a employee
	*/
	public function exists($cust_id)
	{
		$this->db->from('m_karyawan');
		$this->db->where('m_customer.nik', $cust_id);
	...
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'nama_karyawan', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
	...
	*/
	...
	{
		return $this->search($search, 0, 0, 'nama_karyawan', 'asc', TRUE);
	}
	...
	*/
	public function get_info($customer_id)
	{
		...
		$this->db->where('m_customer.cust_id', $customer_id);
		$query = $this->db->get();
		...
		else
		{
			...
			foreach($this->db->list_fields('m_customer') as $field)
			{
				$person_obj->$field = '';
			}
