...
class Report extends CI_Model
{
	...
	public function get_data_kelompok_jenis_pengaduan($sheet, $form_type, $inputTgl1, $inputTgl2, $inputKota, $kategori, $jenis, $gender, $direktorat)
	{
		if ($gender == 'ALL') {
			...
			$query .= " GROUP BY kategori ) t2 on t1.id=t2.kategori 
			LEFT JOIN
			( 
				SELECT 
					kategori, COUNT(*) AS cnt
				FROM
					desk_tickets
				WHERE
					tglpengaduan BETWEEN '$inputTgl1' AND '$inputTgl2' 
			...
			$query .= " GROUP BY kategori) t4 on t1.id=t4.kategori 
				LEFT JOIN
				( 
					SELECT 
						kategori, COUNT(*) AS cnt
			...
			$query .= " GROUP BY kategori) t5 on t1.id=t5.kategori ";
	...
	public function get_data_kelompok_mekanisme_menjawab($sheet, $form_type, $inputTgl1, $inputTgl2, $inputKota, $kategori, $jenis, $gender, $direktorat)
	{
		if ($gender == 'ALL') {
			...
			$query .= " GROUP BY submited_via ) t2 on t1.nama=t2.submited_via 
			LEFT JOIN
			( 
				SELECT 
					submited_via, COUNT(*) AS cnt
				FROM
					desk_tickets
				WHERE
					tglpengaduan BETWEEN '$inputTgl1' AND '$inputTgl2' 
			...
			$query .= " GROUP BY submited_via) t4 on t1.nama=t4.submited_via 
				LEFT JOIN
				( 
					SELECT 
						submited_via, COUNT(*) AS cnt
			...
			$query .= " GROUP BY submited_via) t3 on t1.nama=t3.submited_via 
				LEFT JOIN
				( 
					SELECT 
						submited_via, COUNT(*) AS cnt
			...
			$query .= " GROUP BY submited_via) t5 on t1.nama=t5.submited_via 
			ORDER BY t1.urutan";
		} else {
	...
	public function get_data_jenis_profesi_pengadu($sheet, $form_type, $inputTgl1, $inputTgl2, $inputKota, $kategori, $jenis, $gender, $direktorat)
	{
		if ($gender == 'ALL') {
			...
			$query .= " GROUP BY iden_profesi ) t2 on t1.id=t2.iden_profesi 
			LEFT JOIN
			( 
				SELECT 
					iden_profesi, COUNT(*) AS cnt
				FROM
					desk_tickets
				WHERE
					tglpengaduan BETWEEN '$inputTgl1' AND '$inputTgl2' 
			...
			$query .= " GROUP BY iden_profesi) t4 on t1.id=t4.iden_profesi 
				LEFT JOIN
				( 
					SELECT 
						iden_profesi, COUNT(*) AS cnt
			...
			$query .= " GROUP BY iden_profesi) t5 on t1.id=t5.iden_profesi 
			order by t1.id";
		} else {
			$query = "SELECT t1.*, t2.cnt as cnt_p, t3.cnt as cnt_i 
				FROM desk_profesi t1 
				LEFT JOIN 
				( 
					SELECT iden_profesi, count(*) as cnt 
					FROM desk_tickets 
	...
	public function get_data_kelompok_informasi_produk($sheet, $form_type, $inputTgl1, $inputTgl2, $inputKota, $kategori, $jenis, $gender, $direktorat)
	{
		if ($gender == 'ALL') {
			...
			$query .= " GROUP BY klasifikasi ) t2 on t1.nama=t2.klasifikasi 
			LEFT JOIN
			( 
				SELECT 
					klasifikasi, COUNT(*) AS cnt
				FROM
					desk_tickets
				WHERE
					tglpengaduan BETWEEN '$inputTgl1' AND '$inputTgl2' 
			...
			$query .= " GROUP BY klasifikasi) t4 on t1.nama=t4.klasifikasi 
				LEFT JOIN
				( 
					SELECT 
						klasifikasi, COUNT(*) AS cnt
			...
			$query .= " GROUP BY klasifikasi) t5 on t1.nama=t5.klasifikasi 
			ORDER BY t1.id";
		} else {
			$query = "SELECT 
			t1.nama as name, t2.cnt as cnt_p, t3.cnt as cnt_i 
