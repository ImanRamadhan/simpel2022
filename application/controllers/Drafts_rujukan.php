...
class Drafts_rujukan extends Secure_Controller
{
	...
	*/
	public function search()
	{
		...
		$items = $this->Draft->search_rujukan($search, $filters, $limit, $offset, $sort, $order);

		$total_rows = $this->Draft->get_rujukan_found_rows($search, $filters);

		$data_rows = array();
		$no = $offset;
		foreach($items->result() as $item)
		{
			$data_rows[] = $this->xss_clean(get_drafts_rujukan_data_row($item, ++$no));
