...
<div class="row">
	<div class="col-sm-12 col-lg-12">
		...
		<div class="form-group form-group-sm row">
			<?php echo form_label('', 'attachments', array('class' => 'col-form-label col-sm-2')); ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$.fn.load_attachments2 = function() {
			$.ajax({
				type: 'GET',
				url: "<?php echo site_url($controller_name . '/get_attachments?ticketid=' . (($controller_name == 'tickets') ? $item_info->trackid : $item_info->id) . '&mode=1') ?>",
		...
		$("#dragndrop2").dropzone({
			url: "<?php echo site_url('tickets/upload_lamp_jawaban'); ?>",
			addRemoveLinks: true,
			uploadMultiple: true,
			parallelUploads: 10,
			maxFiles: 10,
			...
			init: function() {
				...
				this.on("sending", function(file, xhr, formData) {
					...
					<?php else: ?>
						formData.append("ticketid", "<?php echo $item_info->id; ?>");
					<?php endif; ?>
				});
