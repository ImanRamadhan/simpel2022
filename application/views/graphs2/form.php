...
$(document).ready(function()
{
	$('#process_btn').on('click', function(e)
	{
		...
		switch($('#grafik').val()){
			...
			case "4":
				$.ajax({
					...
					success: function (Result) {
						var DataChart = JSON.parse(Result);
						var subtitle = 'Tanggal '+ $("#tgl1").val() + ' s.d ' + $("#tgl2").val();
						CreateBarChartV("graphs_holder", DataChart, DataChart[0].name, subtitle, '', '');
			...
			case "11":
				$.ajax({
					...
					data: {
						...
						subklasifikasi_id	: $('#subklasifikasi_id').val(),
						<?php echo $this->security->get_csrf_token_name(); ?>: csrf_token()
					},
					type: 'POST',
					success: function (Result) {
			...
			case "12":
				$.ajax({
					url: '<?php echo site_url('Graphs/statusLayanan') ?>',
					data: {
						tgl1	: $("#tgl1").val(),
						tgl2	: $("#tgl2").val(),
						kota	: $('#kota').val(),
					...
					success: function (Result) {
						var DataChart = JSON.parse(Result);
						var subtitle = 'Tanggal '+ $("#tgl1").val() + ' s.d ' + $("#tgl2").val();
						CreateBarChart("graphs_holder", DataChart, 'Grafik Berdasarkan Status Layanan', subtitle, 'Jenis Produk', 'Jml Layanan');
						$('#table').hide();
					},
					error: function (request, status, error) {
						//alert('error');
	...
	<div class="col-md-12">
		<div class="card ">
			...
			<div class="card-body">
                ...
                <?php 
                    $this->load->view("graphs2/component-graphs-header");
                ?>
				<div class="col-md-10">
...
$(document).ready(function()
{
	$('#kota').on('change', function(){
		$.remember({ name: 'grafik.kota', value: $('#kota').val() })
	});
	
	if($.remember({ name: 'grafik.kota' }) != null)
		$('#kota').val($.remember({ name: 'grafik.kota' }));

    if($.remember({ name: 'grafik.grafik' }) != null)
		$('#grafik').val($.remember({ name: 'grafik.grafik' }));
