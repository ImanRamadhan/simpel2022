(function(table_support4, $) {

	var enable_actions = function(callback) {
		return function() {
			var selection_empty = selected_rows().length == 0;
			$("#toolbar4 button:not(.dropdown-toggle)").attr('disabled', selection_empty);
			typeof callback == 'function' && callback();
		}
	};

	var table = function() {
		return $("#table4").data('bootstrap.table');
	}
	...
	var selected_rows = function () {
		return $("#table4 td input:checkbox:checked").parents("tr");
	};
	...
	var init = function (_options) {
		...
		$('#table4').bootstrapTable($.extend(options, {
...
})(window.table_support4 = window.table_support4 || {}, jQuery);
