(function(table_support5, $) {

	var enable_actions = function(callback) {
		return function() {
			var selection_empty = selected_rows().length == 0;
			$("#toolbar5 button:not(.dropdown-toggle)").attr('disabled', selection_empty);
			typeof callback == 'function' && callback();
		}
	};

	var table = function() {
		return $("#table5").data('bootstrap.table');
	}
	...
	var selected_rows = function () {
		return $("#table5 td input:checkbox:checked").parents("tr");
	};
	...
	var init = function (_options) {
		...
		$('#table5').bootstrapTable($.extend(options, {
	...
	var init_delete = function (confirmMessage) {
		$("#delete5").click(function (event) {
			do_delete();
		});
	};

	var init_restore = function (confirmMessage) {
		$("#restore5").click(function (event) {
			do_restore();
		});
	};
...
})(window.table_support5 = window.table_support5 || {}, jQuery);
