(function(table_support3, $) {

	var enable_actions = function(callback) {
		return function() {
			var selection_empty = selected_rows().length == 0;
			$("#toolbar3 button:not(.dropdown-toggle)").attr('disabled', selection_empty);
			typeof callback == 'function' && callback();
		}
	};

	var table = function() {
		return $("#table3").data('bootstrap.table');
	}
	...
	var selected_rows = function () {
		return $("#table3 td input:checkbox:checked").parents("tr");
	};
	...
	var init = function (_options) {
		...
		$('#table3').bootstrapTable($.extend(options, {
...
})(window.table_support3 = window.table_support3 || {}, jQuery);
